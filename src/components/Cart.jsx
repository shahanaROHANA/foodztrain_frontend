...
const Cart = () => {
  ...
  const calculateSubtotalCents = () => {
    if (!cart || !cart.items) return 0;
    return cart.items.reduce((total, item) => total + item.priceCents * item.quantity, 0);
  };
  ...
  if (currentView === 'checkout') {
    // Build proper checkout payload
    const orderPayload = {
      cartItems: cart?.items || [],
      subtotalCents: calculateSubtotalCents(),
      deliveryCents: 2000,
      finalCents: calculateSubtotalCents() + 2000
    };
    ...
    return (
      <Checkout
        ...
        deliveryCents={orderPayload.deliveryCents}
        finalCents={orderPayload.finalCents}
        onOrderSuccess={handleCheckoutSuccess}
        onBack={handleBackToCart}
      />
    );
  ...
  return (
    <div className="cart-container">
      ...
      <div className="cart-summary">
        <div className="summary-row">
          <span>Subtotal:</span>
          <span>₹{formatPrice(calculateSubtotalCents())}</span>
        </div>
        <div className="summary-row">
          <span>Delivery Fee:</span>
          <span>₹20.00</span>
        ...
        <div className="summary-row total">
          <span>Total:</span>
          <span>₹{formatPrice(calculateSubtotalCents() + 2000)}</span>
        </div>
